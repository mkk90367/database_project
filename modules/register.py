...
def mai(root):
    ...
    recruit = Button(r1, image=r1.Img1, border=0, bg="#03DDEE",
                     relief="raised", activebackground="#03EAFD", command=lambda: rejestracjaDlaRekrutera(root))
    ...
    recruit2 = Button(r1, image=r1.Img2, border=0, bg="#05edFC",
                      relief="raised", activebackground="#05F6FD", command=lambda: rejestracjaDlaKlienta(root))
...
def rejestracjaDlaRekrutera(root):
    global name, email, pwd, cpwd
    print("Witaj rekruterze")
    r1.destroy()
    ...
    btn = Button(r2, image=r2.bn, bg='#FFFFFF', bd=0,
                 activebackground="#ffffff", command=lambda: sprawdzenieRekrutera(root))
...
def sprawdzenieRekrutera(root):
    ...
    if name1 and email1 and pwd1 and cpwd1:
        ...
        total = cur.fetchall()
        mycon.close()
        istniejaceAdresyEmail = []
        for i in total:
            istniejaceAdresyEmail.append(i[0])
        print("uzytkownicy istniejacy w bazie:", istniejaceAdresyEmail)

        if email1 in istniejaceAdresyEmail:
            messagebox.showinfo('UWAGA!', 'EMAIL JEST JUZ ZAREJESTROWANY')
            email.delete(0, END)

        else:
            if pwd1 == cpwd1:
                dokonczenieRekrutera(root)
            else:
...
def dokonczenieRekrutera(root):
    ...
    btn = Button(r3, image=r3.bn, bg='#FFFFFF', bd=0,
                 activebackground="#ffffff", command=lambda: wyslijRekruteraDoBazy(root))
    btn.place(x=320, y=500)

def wyslijRekruteraDoBazy(root):
    ...
    if gender1 and company1 and loc1:
        ...
        try:
            ...
            mycon.commit()
            mycon.close()
            messagebox.showinfo('SUKCES!', 'Rejestracja ukonczona pomyslnie')
            logi(root)
        except:
            pass
...
def rejestracjaDlaKlienta(root):
    ...
    r2.bn = PhotoImage(file="elements\\next1.png")
    btn = Button(r2, image=r2.bn, bg='#FFFFFF', bd=0,
                 activebackground="#ffffff", command=lambda: sprawdzenieKlienta(root))
    btn.place(x=320, y=500)
...
def sprawdzenieKlienta(root):
    ...
    if name1 and email1 and pwd1 and cpwd1:
        ...
        if email1 in istniejaceAdresyEmail:
            messagebox.showinfo('UWAGA!', 'EMAIL ZOSTAL JUZ ZAREJESTROWANY')
            email.delete(0, END)

        else:
            if pwd1 == cpwd1:
                dokonczenieKlienta(root)
            else:
                messagebox.showinfo('UWAGA!', 'HASLA NIE PASUJA')
...
def dokonczenieKlienta(root):
    ...
    r3.bn = PhotoImage(file="elements\\reg.png")
    btn = Button(r3, image=r3.bn, bg='#FFFFFF', bd=0,
                 activebackground="#ffffff", command=lambda: wyslijKlientaDoBazy(root))
    btn.place(x=320, y=550)

def wyslijKlientaDoBazy(root):
    ...
    if gender1 and age1 and loc1 and workxp1:
        ...
        try:
            ...
            mycon.commit()
            mycon.close()
            messagebox.showinfo('SUKCES!', 'Rejestracja zakonczona powodzeniem')
            logi(root)
        except:
            pass
